    ...
    if (opps.length > 0) {
        opps.forEach(opponent => {
            if (opponent > 0) { //id=0は不戦勝なので加算の必要なし
                oppresult.push(partlist[opponent - 1].win / partlist[opponent - 1].opps.length);
            }
        });

        oppresult.forEach(element => {
            omws += element;
        });

        return Math.round(omws * 1000 / opps.length) / 10; //小数点第一位で四捨五入して返す
