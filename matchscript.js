...
/* 以下マッチングタイプごとにメソッドを切り離す
*/

//トーナメント1回戦のみ参加者をランダムにし、プレイヤー数を4の倍数にしてマッチング
    /* 
    *上記2^n乗じゃないといけないので書き換える必要あり
    */
function makeTNFirstRound(array){
    shuffle(array);
        ...
        for (let i = 0; i < numbye; i++) {
            ...
            if (i == 0) {
                ...
                array[(array.length / 2) - 1].id = byeid;
                byeid--;
            }
        }
}
...
function acceptResult(e) {
    ...
    if (results[0] == "勝ち" && results[1] == "勝ち") {
        ...
        if (results[0] == "勝ち" && ids[1] <= 0) {
            //bye(id<=0)に勝利の結果を与える入力も受け付けない
            alert("byeを勝ちにすることはできません。");
        } else if (results[1] == "勝ち" && ids[2] <= 0) {
            alert("byeを勝ちにすることはできません。");
        } else {
